...
export class NTClient {
  ...
  private onTopicAnnounce: (topic: NTTopic) => void;
  private onTopicUnannounce: (topic: NTTopic) => void;
  ...
  private subscriptions: Map<number, NTSubscription> = new Map();
  private publishedTopics: Map<string, NTTopic> = new Map();
  private serverTopics: Map<string, NTTopic> = new Map();
  ...
  constructor(
    serverAddr: string,
    appName: string,
    onTopicAnnounce: (topic: NTTopic) => void,
    onTopicUnannounce: (topic: NTTopic) => void,
    onNewTopicData: (
      topic: NTTopic,
      timestamp_us: number,
      value: unknown
  ...
  setProperties(topic: string, properties: { [id: string]: any }) {
    // Update local topics
    let updateTopic = (toUpdate: NTTopic) => {
  ...
  private ws_subscribe(subscription: NTSubscription) {
    this.ws_sendJSON("subscribe", subscription.toSubscribeObj());
  }

  private ws_unsubscribe(subscription: NTSubscription) {
    this.ws_sendJSON("unsubscribe", subscription.toUnsubscribeObj());
  }

  private ws_publish(topic: NTTopic) {
    this.ws_sendJSON("publish", topic.toPublishObj());
  }

  private ws_unpublish(topic: NTTopic) {
